...
function renderChapterSection(
    ...
    const html =
        imageMap.size > 0
            ? remapImageSrcs(chapter.htmlContent, imageMap)
            : chapter.htmlContent
...
export const renderHtml: RendererFn = async (data, settings) => {
    ...
    const cover = settings.includeCoverImage
        ? await fetchCoverImage(data.core.coverImageUrl)
        : null
    ...
    if (settings.chapterSplit) {
        ...
        for (let index = 0; index < data.core.chapters.length; index++) {
            ...
            const chapterTitle =
                data.core.chapters[index]?.title ?? `Chapter ${index + 1}`
            files[
                `${paddedIndex}-${chapterTitle.replace(/[<>:"/\\|?*]/g, "_").slice(0, 40)}.html`
            ] = htmlPage(
                chapterTitle,
                renderChapterSection(data, index, settings, imageMap),
            )
        }
        return zipFiles(files)
    }

    const tocHtml = settings.includeToc
    ...
    const chapterSections = data.core.chapters
        .map((chapter, index) => {
            const section = renderChapterSection(
                data,
                index,
                settings,
                imageMap,
    ...
    const body = [infoHtml, tocHtml, chapterSections]
        .filter(Boolean)
        .join("\n\n")
