...
async function runDownload(
    ...
    try {
        ...
        const isZip =
            ...
            (settings.chapterSplit && settings.format !== "epub")
        ...
        if (!isCancelled(id)) {
            ...
            console.error(
                `[fanfic-downloader] download failed for job ${id}:`,
                error,
            )
            await updateJob(id, {
                status: "failed",
    ...
    if (state === "interrupted") {
        void (async () => {
            try {
                ...
                console.error(
                    "[fanfic-downloader] handleDownloadChange error:",
                    error,
                )
            }
        })()
